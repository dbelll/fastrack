...
//	printf("dumpResultsAux for standings at %p and vsChamp at %p ... \n", standings, vsChamp);
	for (int iSession = g_p.standings_freq-1; iSession < g_p.num_sessions; iSession += g_p.standings_freq) {
		...
		for (int iAg = 0; iAg < g_p.num_agents; iAg++) {
			...
			fprintf(f, "%d, %d, %d, %d, %d, %d, %d, %d, %f, %f, %d, %f\n", iSession, iAg, standings[iStand].games, standings[iStand].wins, standings[iStand].losses, vsChamp[iStand].games, vsChamp[iStand].wins, vsChamp[iStand].losses, standings[iStand].alpha, standings[iStand].lambda, standings[iStand].training_pieces, standings[iStand].epsilon);
		}
	}
//	printf("done.\n");
}
